...
region_t* extract_requests_matter(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    //Check if region is at least 8 bytes (the message header size of Matter protocol)
    if (mem_count >= 8) {
      // Byte 1: Message Flags
      // Bytes 2 - 3: Session ID
      // Byte 4: Security Flags
      // Byte 5 - 8: Message Counter
      
      // Extract message 
      u64 message_flags = mem[0];
      u64 security_flags = mem[3];

      if (((message_flags == 0x04) || 
           (message_flags == 0x03) ||
           (message_flags == 0x02) ||
           (message_flags == 0x01) ||
           (message_flags == 0x00)) && (security_flags == 0x00)) {
...
unsigned int* extract_response_codes_matter(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref)
{
  ...
  while (byte_count < buf_size) {
    ...
    if (mem_count >= 8) {
      ...
      if (((message_flags == 0x01) || (message_flags == 0x00)) && (security_flags == 0x00)) {
        state_count++;
        state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
        state_sequence[state_count - 1] = message_code;
        mem_count = 0;
